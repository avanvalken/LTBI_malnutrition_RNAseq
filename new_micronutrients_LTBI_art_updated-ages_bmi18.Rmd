...
# question: why log_cpm instead of log_combatseq?
```
...
plotPCA(DESeqTransform(indata_tmp), intgroup = "bmi_cat2")
```
...
mat = as.matrix(assay(indata[row.names(top_1k_genes),],"log_cpm"))
mat = t(scale(t(mat)))
...
Heatmap(mat,show_row_names=F,show_column_names = T, top_annotation = ha, cluster_columns = F )
```




# 2 Differential expression, bmi_cat2 and age {.tabset}
```{r diffex2}
#designMat <- model.matrix(~factor(Tb_status) + factor(bmi_cat2) + factor(Tb_status):factor(bmi_cat2), data=colData(indata))

#designMat <- model.matrix(~factor(Tb_status) + factor(bmi_cat2), data=colData(indata))

designMat <- model.matrix(~factor(bmi_cat2) + as.numeric(age), data=colData(indata))

colnames(designMat)
colnames(designMat) <- c("Intercept", "wellnourished", "age")
...
both_sig_limmaResNut <- limmaResNut[limmaResNut$adj.P.Val <0.05,]
both_sig_limmaResNut <- both_sig_limmaResNut[order(both_sig_limmaResNut$adj.P.Val),]
...
```{r 2diffex-datatable-heatmap2}
datatable(both_sig_limmaResNut, 
          options=list(scrollX=T,pageLength=20),
          rownames = T)
...
# 3 Differential expression, age only {.tabset}
```{r diffex3}
#designMat <- model.matrix(~factor(Tb_status) + factor(bmi_cat2) + factor(Tb_status):factor(bmi_cat2), data=colData(indata))

#designMat <- model.matrix(~factor(Tb_status) + factor(bmi_cat2), data=colData(indata))

designMat <- model.matrix(~ as.numeric(age), data=colData(indata))

colnames(designMat)
colnames(designMat) <- c("Intercept", "age")
...
contrast.matrixNut<- makeContrasts(age, levels = designMat)
fitNut <- contrasts.fit(fit,contrast.matrixNut)
fitNut <- eBayes(fitNut)
limmaResNut <- topTable(fitNut, adjust.method = "BH", n = Inf, sort.by = "P")
...
AGE_only_sig_limmaResNut <- limmaResNut[limmaResNut$adj.P.Val <0.05,]
AGE_only_sig_limmaResNut <- AGE_only_sig_limmaResNut[order(AGE_only_sig_limmaResNut$adj.P.Val),]

#write.csv(sig_limmaResNut, "sig_TBgenes_padj.csv")

top_500_genes <- limmaResNut[1:500,]
top_1k_genes <- limmaResNut[1:1000,]
top_5k_genes_age <- limmaResNut[1:5000,]
## top 50 by logfc
###top 50 by p.adj value
AGE_only_top_50 <- sig_limmaResNut[1:50,]
#write.csv(top_50, "top_50_by_padj.csv")
...
## All genes datatable and heatmap {.tabset}
```{r diffex-datatable-heatmap3}
datatable(AGE_only_sig_limmaResNut, 
          options=list(scrollX=T,pageLength=20),
          rownames = T)
...
## compare age and bmi_cat2 genelists

Because age is significantly different between the two groups, we analyzed only genes specific to differences in BMI categories by calculating differentially expressed genes with limma using either age or BMI categories as a cofacter and subtracting from the BMI list anything that showed up in the age differential expression results. 

These results were uploaded to Qiagen's ingenuity database to estimate which pathways were most affected by nutrition status. 

**we could also do age here**
**also, does age need to be binary?**


```{r}

# get a bmispecific genelist out of the top 5k genes from each list
bmi <- rownames(top_5k_genes_bmi_cat2)
age_genes <- rownames(top_5k_genes_age)




bmi_only <- setdiff(bmi, age_genes)
age_only <- setdiff(age_genes, bmi)

```

There are `r length(bmi_only)` DEG's unique to BMI category. 
...
real_genes <- genecheck(rownames(top_1k_genes),databases)
```
...
```{r enriched-pathways, eval=FALSE}
...
cp <- as.data.frame(t(rbind(zak16_cp[,1:13], predict29_cp[,1:13], risk4_cp[,1:13], sweeney3_cp[,1:13])))

colnames(cp) <- c("ZAK16", "PREDICT29", "RISK4", "SWEENEY3")
